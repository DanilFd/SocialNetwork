...
import {deleteLoginThunk, getAuthThunk} from '../../redux/asyncActions/auth';
    ...
    return (
        <header className="sticky-top d-flex justify-content-between" style={{backgroundColor: "#332F2C"}}>
            ...
            {!userData ?
                ...
                :
                <div className="d-flex justify-content-around align-items-center text-white pe-5">
                    <img style={{width: 35, height: 35, borderRadius: "50%", marginRight: 25}} src={userPhotoUrl || ""}
                         alt=""/>
                    <span className="pe-5">{userData.login}</span>
                    <button onClick={() => dispatch(deleteLoginThunk())} className="btn btn-primary">Logout</button>
                </div>
            }
        </header>
    );
